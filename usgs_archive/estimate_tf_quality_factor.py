...
class EMTFStats(object):
    ...
    def __init__(self, tf_dir=None, *args, **kwargs):
        ...
        self.types = ['{3}_{0}{1}_{2}'.format(ii, jj, kk, ll) 
                     ...
                     ['bad_points_{2}_{0}{1}'.format(ii, jj, ll) 
                      ...
                      for jj in ['x', 'y']  
                      for ll in ['res', 'phase']] + \
                     ['bad_points_tipper_{0}'.format(ii) 
                      for ii in ['x', 'y']]
    ...
    def locate_bad_res_points(self, res):
        ...
        m_index = np.where((abs(res-med) >= tol*.95) &
                           (abs(res-med) <= tol*1.05))[0][0]
        r_index = m_index + 1
    ...
    def locate_bad_phase_points(self, phase, test=5):
        ...
        m_index = np.where((abs(phase-med) >= tol*.95) & 
                           (abs(phase-med) <= tol*1.05))[0][0]
        r_index = m_index + 1
    ...
    def locate_bad_tipper_points(self, tipper, test=.2):
        ...
        m_index = np.where((abs(tipper-med) >= tol*.95) & 
                           (abs(tipper-med) <= tol*1.05))[0][0]
        r_index = m_index + 1
        ...
        for kk, edi in enumerate(edi_list):
            ...
            for ii in range(2):
                for jj in range(2):
                    ...
                    ### locate bad points
                    bad_points_res = self.locate_bad_res_points(mt_obj.Z.resistivity[:, ii, jj])
                    stat_array[kk]['bad_points_res_{0}'.format(comp)] = max([1, len(bad_points_res)])
                    bad_points_phase = self.locate_bad_phase_points(mt_obj.Z.phase[:, ii, jj])
                    stat_array[kk]['bad_points_phase_{0}'.format(comp)] = max([1, len(bad_points_res)])
                    ...
                    stat_array[kk]['res_{0}_diff'.format(comp)] = np.abs(np.median(np.diff(res)))
                    stat_array[kk]['phase_{0}_diff'.format(comp)] = np.abs(np.median(np.diff(phase)))
                    
                    ### compute tipper
                    if ii == 0:
                        tcomp = self.t_dict[(0, jj)]
                        t_index = np.nonzero(mt_obj.Tipper.amplitude[:, 0, jj])
                        bad_points_t = self.locate_bad_tipper_points(mt_obj.Tipper.amplitude[:, 0, jj])
                        stat_array[kk]['bad_points_tipper_{0}'.format(tcomp)] = max([1, len(bad_points_t)])
