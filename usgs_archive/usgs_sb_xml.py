...
class XMLMetadata(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.udom = (
            "Station identifier of MT sounding used to distinguish "
            + "between the soundings associated with this survey."
        )
        self.lat_def = (
            "Latitude coordinate of station referenced to the "
            + "World Geodetic Service Datum of 1984 (WGS84)."
        )
        self.lon_def = (
            "Longitude coordinate of station, referenced to the "
            + "World Geodetic Service Datum of 1984 (WGS84)"
        )
        self.elev_def = (
            "Elevation, referenced to the North American "
    ...
    def _set_id_info(self):
        ...
        ET.SubElement(
            citeinfo, "othercit"
        ).text = "Additional information about Originator: {0}".format(
            ", ".join(self.authors)
        )
        ...
        if self.journal_citation:
            ...
            ET.SubElement(jciteinfo, "pubdate").text = self._get_date(
                self.journal_citation.date
            )
            ...
            if len(self.journal_citation.orcid) > 0:
                orcid_str = ", ".join(
                    [
                        "{0}, https://orcid.org/{1}".format(author, orcnum)
                        for author, orcnum in zip(
                            self.journal_citation.author, self.journal_citation.orcid
                        )
        ...
        for name in ["westbc", "eastbc", "northbc", "southbc"]:
            ET.SubElement(bounding, name).text = "{0:.5f}".format(
                getattr(self.survey, name[:-2])
            )
        ...
        ET.SubElement(
            place, "placekt"
        ).text = "Geographic Names Information System (GNIS)"
        ...
        # funding source
        try:
            ET.SubElement(idinfo, "datacred").text = self.program_dict[
                self.submitter.funding_source
            ]
        except KeyError:
    ...
    def _set_data_quality(self):
        ...
        ET.SubElement(accuracy, "attraccr").text = (
            "No formal attribute accuracy " + "tests were conducted."
        )
        ET.SubElement(data_quality, "logic").text = (
        ...
        for step in range(1, step_num + 1, 1):
            ...
            ET.SubElement(processing_step, "procdesc").text = getattr(
                self.processing, "step_{0:02}".format(step)
            )
            if step == 1:
                ET.SubElement(processing_step, "procdate").text = self._get_date(
                    self.survey.begin_date
                )
            else:
                ET.SubElement(processing_step, "procdate").text = self._get_date(
                    getattr(self.processing, "date_{0:02}".format(step))
                )
    ...
    def _set_extra_info(self, station=False):
        ...
        if station is False:
            ...
            ET.SubElement(
                station_attr, "attrdef"
            ).text = "Individual station name within MT survey."
            ...
            ET.SubElement(lat_rdom, "rdommin").text = "{0:.5f}".format(
                self.survey.south
            )
            ET.SubElement(lat_rdom, "rdommax").text = "{0:.5f}".format(
                self.survey.north
            )
            ...
            ET.SubElement(elev_rdom, "rdommin").text = "{0:.1f}".format(
                self.survey.elev_min
            )
            ET.SubElement(elev_rdom, "rdommax").text = "{0:.1f}".format(
                self.survey.elev_max
            )
            ...
            ET.SubElement(
                hx_azm_attr_dom, "udom"
            ).text = "Angle (degrees) relative to geographic North."
            ...
            ET.SubElement(
                id_attr, "attrdef"
            ).text = "Data logger or instrument setup ID."
            ...
            ET.SubElement(
                dt_attr_dom, "udom"
            ).text = "W = wideband, L = long-period, Bb = broadband"
            ...
            dq_str = ": ".join(
                [
                    "5 = great TF from 10 to 10000 secs (or) longer",
    ...
    def _set_distribution_info(self):
        ...
        for key in ["address", "city", "state", "postal", "country"]:
            ET.SubElement(center_address, key).text = getattr(self.science_base, key)
    ...
    def _set_meta_info(self):
        ...
        ET.SubElement(meta_info, "metd").text = datetime.datetime.now().strftime(
            "%Y%m%d"
        )
        ...
        ET.SubElement(meta_info, "metstdn").text = (
