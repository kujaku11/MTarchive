...
with usgs_archive.Capturing() as output:
    ...
    if copy_files:
        if not station_save_dir.joinpath(f'{station}.edi').is_file():
            shutil.copy(edi_path.joinpath(f'{station}.edi'.format(station)),
                        station_save_dir.joinpath(f'{station}.edi'))
        if not station_save_dir.joinpath(f'{station}.png').is_file():
            shutil.copy(edi_path.joinpath(f'{station}.png'.format(station)),
    ...
    # write station xml
    s_xml.write_xml_file(station_save_dir.joinpath('{0}_meta.xml'.format(station)), 
                        write_station=True)
...
if upload_data:
    try:
        usgs_archive.sb_upload_data(page_id, 
                                    station_save_dir, 
                                    username,
                                    password,
